...
using Trijinx.HLE.HOS.Services.Sockets.Bsd;
using Trijinx.HLE.HOS.Services.Sockets.Bsd.Impl;
using Trijinx.HLE.HOS.Services.Sockets.Bsd.Proxy;
using Trijinx.HLE.HOS.Services.Ssl.Types;
using System;
using System.IO;
using System.Net;
...
namespace Trijinx.HLE.HOS.Services.Ssl.SslService
{
...
#pragma warning restore SYSLIB0039
        ...
        {
            // The Nintendo Switch does not perform strict certificate validation
            // the way desktop OS SSL stacks do. Accept all certificates to match
            // Switch behaviour and avoid platform-specific failures (e.g. macOS
            // Secure Transport rejecting certificates that Windows accepts).
            return true;
        }
        ...
        public ResultCode Handshake(string hostName)
        {
            ...
            try
            {
                _stream = new SslStream(
                    new NetworkStream(((DefaultSocket)((ManagedSocket)Socket).Socket).BaseSocket, false),
                    false,
            ...
            {
                Logger.Error?.Print(LogClass.ServiceSsl, $"SSL handshake failed: {ex.Message}");
                EndSslOperation();
            ...
            {
                Logger.Error?.Print(LogClass.ServiceSsl, $"SSL handshake socket error: {socketEx.Message}");
                EndSslOperation();

                WsaError socketErrorCode = (WsaError)socketEx.SocketErrorCode;
            ...
            {
                Logger.Error?.Print(LogClass.ServiceSsl, $"SSL handshake error: {ex.Message}");
                EndSslOperation();
