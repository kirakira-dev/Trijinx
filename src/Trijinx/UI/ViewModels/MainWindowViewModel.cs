...
using Trijinx.Ava.UI.Models.Generic;
using Trijinx.Ava.UI.Renderer;
using Trijinx.Ava.UI.Views.Dialog;
...
using Trijinx.HLE.HOS.Services.Nfc.AmiiboDecryption;
...
using ShaderCacheLoadingState = Trijinx.Graphics.Gpu.Shader.ShaderCacheState;
...
#pragma warning restore IDE0055
                ...
                await Dispatcher.UIThread.InvokeAsync(async () =>
                {
                    await ContentDialogHelper.ShowTextDialog(
                        LocaleManager.Instance[
                            numAdded > 0 || numRemoved > 0 ? LocaleKeys.TrijinxConfirm : LocaleKeys.TrijinxInfo],
                        msg,
                        string.Empty,
                        string.Empty,
                        string.Empty,
        ...
        public async Task InstallFirmwareFromFile()
        {
            Optional<IStorageFile> result = await StorageProvider.OpenSingleFilePickerAsync(new FilePickerOpenOptions
            {
                FileTypeFilter = new List<FilePickerFileType>
                {
                    ...
                    {
                        Patterns = ["*.xci", "*.zip"],
                        AppleUniformTypeIdentifiers = ["com.trijinx.xci", "public.zip-archive"],
                        MimeTypes = ["application/x-nx-xci", "application/zip"],
                    },
                    new("XCI")
                    {
                        Patterns = ["*.xci"],
                        AppleUniformTypeIdentifiers = ["com.trijinx.xci"],
        ...
        public async Task ManageProfiles()
        {
            await NavigationDialogHost.Show(AccountManager, ContentManager, VirtualFileSystem,
                LibHacHorizonManager.TrijinxClient);
        }
        ...
        public async Task OpenFile()
        {
            Optional<IStorageFile> result = await StorageProvider.OpenSingleFilePickerAsync(new FilePickerOpenOptions
            {
                ...
                FileTypeFilter = new List<FilePickerFileType>
                {
                    ...
                    {
                        Patterns = ["*.nsp"],
                        AppleUniformTypeIdentifiers = ["com.trijinx.nsp"],
                    ...
                    {
                        Patterns = ["*.nca"],
                        AppleUniformTypeIdentifiers = ["com.trijinx.nca"],
                        MimeTypes = ["application/x-nx-nca"],
                    },
                    new("NRO")
                    {
                        Patterns = ["*.nro"],
                        AppleUniformTypeIdentifiers = ["com.trijinx.nro"],
                        MimeTypes = ["application/x-nx-nro"],
                    },
                    new("NSO")
                    {
                        Patterns = ["*.nso"],
                        AppleUniformTypeIdentifiers = ["com.trijinx.nso"],
            ...
            else
            {
                switch (operationOutcome)
                {
                    case XCIFileTrimmer.OperationOutcome.Successful:
                        TrijinxApp.MainWindow.LoadApplications();
                        break;
                }
            }
        }
