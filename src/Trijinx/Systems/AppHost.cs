...
using LibHac.Common;
using LibHac.Ns;
using Trijinx.Audio.Backends.Dummy;
using Trijinx.Audio.Backends.OpenAL;
using Trijinx.Audio.Backends.SDL3;
using Trijinx.Audio.Backends.SoundIo;
using Trijinx.Audio.Integration;
using Trijinx.Ava.Common;
using Trijinx.Ava.Common.Locale;
using Trijinx.Ava.Input;
...
using Trijinx.Common.Configuration.Multiplayer;
using Trijinx.Common.Logging;
using Trijinx.Common.SystemInterop;
using Trijinx.Common.UI;
using Trijinx.Common.Utilities;
using Trijinx.Graphics.GAL;
using Trijinx.Graphics.GAL.Multithreading;
using Trijinx.Graphics.Gpu;
using Trijinx.Graphics.OpenGL;
using Trijinx.Graphics.Vulkan;
using Trijinx.HLE.FileSystem;
using Trijinx.HLE.HOS;
using Trijinx.HLE.HOS.Services.Account.Acc;
using Trijinx.Input;
using Trijinx.Input.HLE;
using SkiaSharp;
...
using static Trijinx.Ava.UI.Helpers.Win32NativeInterop;
using AntiAliasing = Trijinx.Common.Configuration.AntiAliasing;
using InputManager = Trijinx.Input.HLE.InputManager;
using IRenderer = Trijinx.Graphics.GAL.IRenderer;
using Key = Trijinx.Input.Key;
using MouseButton = Trijinx.Input.MouseButton;
using ScalingFilter = Trijinx.Common.Configuration.ScalingFilter;
using Size = Avalonia.Size;
using Switch = Trijinx.HLE.Switch;
using VSyncMode = Trijinx.Common.Configuration.VSyncMode;

namespace Trijinx.Ava.Systems
{
    internal class AppHost
    {
                    ...
                    if (SetupValidator.CanFixStartApplication(ContentManager, ApplicationPath, userError, out firmwareVersion))
                    {
                        ...
                        if (userError is UserError.NoFirmware)
                        {
                            ...
                            await ContentDialogHelper.CreateInfoDialog(
                                ...
                                string.Empty,
                                LocaleManager.Instance[LocaleKeys.TrijinxInfo]);
                        }
                    }
                    else
                    {
                    ...
                    default:
                        {
                            ...
                            {
                                Logger.Error?.Print(LogClass.Application, "The specified file is not supported by Trijinx.");
        ...
        private void RenderLoop()
        {
            ...
            Dispatcher.UIThread.Post(() => TrijinxApp.MainWindow.LoadApplications());
        }
