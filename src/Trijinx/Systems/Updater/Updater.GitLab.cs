...
using Trijinx.Systems.Update.Client;
using Trijinx.Systems.Update.Common;
using System;
    ...
    internal static partial class Updater
    {
        ...
        private static async Task<Return<VersionResponse>> QueryLatestVersionAsync()
        {
            _updateClient ??= UpdateClient.Builder()
                .WithServerEndpoint("https://update.trijinx.app") // This is the default, and doesn't need to be provided; it's here for transparency.
        ...
        public static async Task<Optional<(Version Current, Version Incoming)>> CheckVersionAsync(bool showVersionUpToDate = false)
        {
            if (!Version.TryParse(Program.Version, out Version currentVersion))
            {
                Logger.Error?.Print(LogClass.Application,
                    $"Failed to convert the current {TrijinxApp.FullAppName} version!");
            ...
            {
                Logger.Error?.Print(LogClass.Application,
                    $"Failed to convert the received {TrijinxApp.FullAppName} version from the update server!");
